--[[
    Obsidian UI - Crimson restyle
    Fork of Obsidian by deividcomsono: https://github.com/deividcomsono/Obsidian

    The API is unchanged, so any script written for upstream Obsidian works as-is:
    only its loadstring URL needs to point here. To retheme, edit Library.Scheme below.
]]
...
--// Capability guard \\--
-- Calling into a game's own ModuleScripts can strip the executor thread's capabilities while
-- getthreadidentity() still reports 8, and the next UI write then fails with "lacking capability
-- Plugin". Every public entry point (see GuardObject at the bottom) and every return from a user
-- callback re-applies identity 8, so scripts built on this library never have to.
...
function Library:SafeCallback(Func: (...any) -> ...any, ...: any)
    ...
    -- The callback may have called game code that dropped this thread's capabilities.
    Elevate()
...
function Library:Notify(...)
    ...
    -- Accent edge down the leading side of the card.
    ...
    -- Every toast gets an icon badge; notifications without a BigIcon fall back to a bell.
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        --// Drop shadow: a sibling of the window, because Sibling ZIndex draws children above parents \\--
        ...
        --// Sidebar panel: full-height, darker, rounded on the left only \\--
        ...
        --// Brand: logo badge, bold title and the footer text as a subtitle \\--
        ...
        --// Page title: the active tab's name, shown large over the content \\--
        ...
        --// Window controls: minimize collapses to the header, close hides to the floating button \\--
        ...
        --// Bottom Bar: kept only as the resize grip in the corner; the footer text moved under the title \\--
        ...
        if WindowInfo.ShowProfile then
            ...
            ProfileAvatar = New("ImageLabel", {
                ...
                BackgroundColor3 = "BackgroundColor",
                BackgroundTransparency = 0,
                -- rbxthumb loads without a yielding GetUserThumbnailAsync call.
    ...
    function Window:AddTab(...)
        ...
        function Tab:AddGroupbox(Info)
            ...
            do
                ...
                if BoxIcon then
                    -- Icon sits in a tinted rounded badge instead of floating bare.
    ...
    function Window:Toggle(Value: boolean?)
        ...
        -- Re-applying the current state would re-cache the faded-out transparencies (all 1s)
        -- and leave the window invisible on the next open, so explicit no-ops stop here.
        ...
        if Library.Animations and Library.Animations.ToggleWindow == true then
            ...
            -- Slide up into place on open, drift down on close.
            if Library.Toggled then
    ...
    function Window:SetCollapsed(State: boolean)
        ...
        -- The profile card is pinned to the bottom edge, so it would ride up over the brand.
        if ProfileCard then
    ...
    --// Floating reopen button, shown while the window is hidden \\--
    if WindowInfo.FloatingButton then
        ...
        -- A press that barely moved is a click; anything further was a drag.
        local PressedAt
...
--// Capability guard: wrap the public API \\--
-- Each public method calls Elevate() before running, and any table it returns (Window, Tab,
-- Groupbox, Toggle, Label...) gets the same treatment, so the whole API chain is covered no
-- matter what game code the caller ran just before.
do
    local GuardedFunctions = setmetatable({}, { __mode = "k" })
    local GuardedObjects = setmetatable({}, { __mode = "k" })

    -- Pure helpers never touch UI and some run every frame, so they stay unwrapped.
    -- Callback/Changed hold the user's own functions; SafeCallback re-elevates after those.
    local Skip = {
        Validate = true,
        SafeCallback = true,
        GiveSignal = true,
    ...
    -- CreateWindow and CreateLoading add methods to Library at runtime (Library:Toggle), so
    -- rescan it after they run.
